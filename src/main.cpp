...
#if defined(NDEBUG)
# error "Bussycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x31620124045d2ca8dba96e69a4faefa5468747e79fa097196a0d1573df4188af");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bussycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bussycoin Signed Message:\n";
...
{
    // Bussycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CTransaction::CheckTransaction(CValidationState &state) const
{
    ...
    if (IsCoinBase())
    {
        if (vin[0].scriptSig.size() < 2 || vin[0].scriptSig.size() > 300) // 100 was being a cuck, so I changed this shit to 300
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 69000 blocks, which will occur approximately never
    nSubsidy >>= (nHeight / 69000); // Bussycoin: 69K blocks in god knows how long

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.69 * 24 * 60 * 60; // Bussycoin: 1.69 days
static const int64 nTargetSpacing = 6.9 * 60; // Bussycoin: 6.9 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bussycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bussycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8c5d36164bff3082dc4642a61285332990d816e451741f238ea245c3d5cc9bac");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Vault Times 22/Feb/2021 Runescape Life invents Bussy Coin, His networth continues to sink further below 0";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 69 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d65071bafbb1a853c43d31bcd35160f50c895f8c3319881cfa544be46ac48f0187ad72a0712e6849b671ebc0102242dc659febb32ce1990435afebe851374fd4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1614056589;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084762399;

        if (fTestNet)
        {
            block.nTime    = 1614056556;
            block.nNonce   = 386331373;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x87e30667e191d975da8dd255fa4e865399601c5e5921e9b51441f887e684f48b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf9, 0xc6, 0xb9, 0xd6 }; // Bussycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BussycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BussycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BussycoinMiner(CWallet *pwallet)
{
    printf("BussycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bussycoin-miner");
    ...
    try { loop {
        ...
        printf("Running BussycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BussycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BussycoinMiner, pwallet));
}
